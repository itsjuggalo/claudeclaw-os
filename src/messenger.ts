  ...
  try {
    ...
    if (agentId === 'main') {
      ...
      if (autoSuggestEnabled) {
        ...
        const refreshSuggestions = (when: string) => {
          void import('./agent-suggestions.js')
            .then(({ refreshAgentSuggestions }) => refreshAgentSuggestions())
            .then((r) => {
              logger.info({ inserted: r.inserted, skipped: r.skipped, reason: r.reason }, `Agent suggestions refreshed (${when})`);
            })
            .catch((err) => {
              logger.warn({ err: err instanceof Error ? err.message : err }, `Agent suggestion refresh failed (${when}, non-fatal)`);
            });
        };
        setTimeout(() => refreshSuggestions('startup'), 10 * 60 * 1000);
        setInterval(() => refreshSuggestions('24h'), 24 * 60 * 60 * 1000);
      } else {
    ...
    const shutdown = async () => {
      ...
      try {
        ...
        // Telegram's stop() makes a final getUpdates call to confirm the offset.
        // A transient network error (ETIMEDOUT / FetchError) here must NOT
        // create an unhandled rejection — that would exit non-zero and trigger
        // a PM2 crash-restart loop.
        logger.warn({ err }, 'messenger.stop() threw during shutdown (non-fatal, continuing exit)');
      }
      process.exit(0);
    };
    ...
    // A transport startup failure (ETIMEDOUT / 409 conflict / network blip — e.g.
    // after gen-mode SIGSTOPs the process during a GPU gen) must NOT crash the
    // whole process: startDashboard() above is already serving the gallery/UI and
    // keeps the process alive, so catch here instead of falling through to the
    // fatal handler and crash-looping the dashboard.
    try {
      await messenger.start(interactive);
    } catch (err) {
      setTelegramConnected(false);
      logger.error({ err, messenger: messenger.kind }, 'Messenger failed to start — dashboard stays up WITHOUT it (gallery/UI unaffected)');
    }
  } catch (err: unknown) {
