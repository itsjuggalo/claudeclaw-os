...
function Stat({ label, value, tone }: { label: string; value: string | number; tone?: string }) {
  return (
    <div class="rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] px-4 py-3">
      <div class="text-[13px] font-medium uppercase tracking-wider text-[var(--color-text-faint)]">{label}</div>
...
function StatePill({ state }: { state: LifecycleRow['state'] }) {
  ...
  return <span class="inline-flex items-center rounded px-1.5 py-0.5 text-[13px] font-medium" style={`color:${c};background:color-mix(in srgb, ${c} 14%, transparent)`}>{label}</span>;
}
    ...
    <div class="flex flex-col h-full">
      <PageHeader
        ...
        actions={
          <button type="button" onClick={() => void spin(refresh)} disabled={refreshing} aria-busy={refreshing} title="Refresh"
            class="inline-flex items-center gap-1.5 rounded-md border border-[var(--color-border)] px-3 py-1.5 text-[14px] font-medium text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-elevated)] hover:text-[var(--color-text)] disabled:opacity-40">
      ...
      {data && (
        <div class="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          {actMsg && (
            <div class="rounded-md border border-[var(--color-border)] bg-[var(--color-elevated)] px-3 py-2 text-[15px] text-[var(--color-text-muted)]">{actMsg}</div>
          )}
          ...
          <section class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface,var(--color-elevated))] p-4">
            <div class="flex items-center justify-between flex-wrap gap-2">
              <div class="flex items-center gap-2">
                ...
                <h2 class="text-[16px] font-semibold">Stale-account reaper</h2>
                {r && (r.live
                  ? <span class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[13px] font-semibold" style="color:var(--color-status-failed);background:color-mix(in srgb,var(--color-status-failed) 14%,transparent)"><ShieldAlert size={13}/> LIVE — deletes</span>
                  : <span class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[13px] font-semibold" style="color:var(--color-status-done);background:color-mix(in srgb,var(--color-status-done) 14%,transparent)"><ShieldCheck size={13}/> dry-run</span>)}
                {r && !r.on && <span class="text-[13px] text-[var(--color-text-faint)]">(sweep off)</span>}
              </div>
              <div class="text-[14px] text-[var(--color-text-faint)] tabular-nums">
            ...
            <div class="mt-3 flex items-end gap-4 flex-wrap">
              <label class="flex flex-col gap-1 text-[14px] text-[var(--color-text-muted)]">
                ...
                <input type="number" min={1} value={warnDays} onInput={(e) => setWarnDays(parseInt((e.target as HTMLInputElement).value, 10) || 0)}
                  class="w-24 rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1 text-[15px] text-[var(--color-text)] tabular-nums" />
              </label>
              <label class="flex flex-col gap-1 text-[14px] text-[var(--color-text-muted)]">
              ...
              <label class="flex items-center gap-2 text-[15px] text-[var(--color-text-muted)] pb-1.5">
                <input type="checkbox" checked={on} onChange={(e) => setOn((e.target as HTMLInputElement).checked)} /> sweep on
              </label>
              <label class="flex items-center gap-2 text-[15px] pb-1.5" style={live ? 'color:var(--color-status-failed)' : 'color:var(--color-text-muted)'}>
              ...
              <button type="button" onClick={applyReaper} disabled={busy === 'reaper'}
                class="ml-auto rounded-md px-3.5 py-1.5 text-[15px] font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-50"
                style="background:var(--color-accent)">
                {busy === 'reaper' ? 'Applying…' : 'Apply'}
              </button>
            </div>
            <p class="mt-2 text-[14px] text-[var(--color-text-faint)] leading-snug">
          ...
          {/* Lifecycle table */}
          <section>
            <h2 class="text-[16px] font-semibold text-[var(--color-text)] mb-2">
            ...
            {data.lifecycle.length === 0 ? (
              <div class="rounded-lg border border-dashed border-[var(--color-border)] px-4 py-6 text-center text-[15px] text-[var(--color-text-faint)]">
                  ...
                  <thead class="sticky top-0 z-10 bg-[var(--color-elevated)] border-b border-[var(--color-border)] text-left">
                    <tr>
                      {['Account', 'State', 'Idle', 'Delete in', 'Bookings', ''].map((h, i) => (
                        <th key={i} class={'px-3 py-2.5 font-medium text-[13px] uppercase tracking-wider text-[var(--color-text-faint)]' + (i === 3 ? ' text-center' : '')}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={row.id} class="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--color-elevated)] transition-colors">
                          <td class="px-3 py-2.5">
                            ...
                            {row.name && <div class="text-[13px] text-[var(--color-text-faint)]">{row.name}</div>}
                          </td>
                          ...
                          <td class="px-3 py-2.5 whitespace-nowrap text-right">
                            <button type="button" onClick={() => keep(row)} disabled={busy === row.id}
                              class="inline-flex items-center gap-1 rounded px-2 py-1 text-[14px] text-[var(--color-text-muted)] hover:text-[var(--color-status-done)] hover:bg-[var(--color-elevated)] disabled:opacity-40">
                              <RotateCcw size={14} /> Keep
                            </button>
                            <button type="button" onClick={() => del(row)} disabled={busy === row.id}
                              class="inline-flex items-center gap-1 rounded px-2 py-1 text-[14px] text-[var(--color-text-muted)] hover:text-[var(--color-status-failed)] hover:bg-[var(--color-elevated)] disabled:opacity-40">
