...
export function PortfolioAIPage() {
  ...
  const flowMap = new Map<string, FlowPick>(flowPicks.map(p => [p.ticker.toUpperCase(), p]));
  const momMap = new Map<string, MomentumEntry>(momentum.map(m => [m.ticker.toUpperCase(), m]));
